...
function Definitions()
	...
	Valid_Units_List = {"X-Wing","Wedge_XWing_Rogue","Rogue_2_Xwing","Rogue_4_Xwing","Rogue_4_Xwing","Rogue_7_Xwing","Rogue_10_XWing","Rogue_11_XWing"}
...
function Get_Squad_Units()
	...
	for each, xwing in pairs(Valid_Units_List) do
		local Found_Units = Find_All_Objects_Of_Type(xwing)
	...
	if Any_In_Nebula(units) then
		for key, unit in pairs(units) do
			if TestValid(unit) and unit.Get_Hull() > 0.0 then
				...
				DebugMessage("%s -- Removed dead X-Wing from table", tostring(Script))
			end
		end
	else
		DebugMessage("%s -- No X-Wing inside a nebula in this squadron", tostring(Script))
	end
end
