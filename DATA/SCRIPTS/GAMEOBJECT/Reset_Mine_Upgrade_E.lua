...
function State_Init(message)
	if message == OnEnter then
		upgrades_list_1 = Find_All_Objects_Of_Type("ES_Increased_Supplies_L1_Upgrade")
		upgrades_list_2 = Find_All_Objects_Of_Type("ES_Increased_Supplies_L2_Upgrade")
		...
		for i, upgrade in ipairs(upgrades_list_1) do
			if TestValid(upgrade) then
				...
				if player_team == upgrade_team then
					Create_Generic_Object("ES_Increased_Supplies_L1_Upgrade", upgrade.Get_Position(), upgrade.Get_Owner())
					upgrade.Despawn()
				end
			end
		end
		...
		for i, upgrade in ipairs(upgrades_list_2) do
			if TestValid(upgrade) then
				upgrade_team = upgrade.Get_Owner().Get_Team()			
				if player_team == upgrade_team then
					Create_Generic_Object("ES_Increased_Supplies_L2_Upgrade", upgrade.Get_Position(), upgrade.Get_Owner())
					upgrade.Despawn()
				end
			end
		end
